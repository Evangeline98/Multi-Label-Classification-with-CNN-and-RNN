...
checkpoint = 'checkpoint_attention1.pth.tar'# path to checkpoint, None if none
    ...
    with torch.no_grad():
        ...
        for i, (imgs, caps, caplens) in enumerate(val_loader):
            ...
            for image_index in range(batch_size):    
                seq,_  = caption_image_beam_search(encoder, decoder, imgs[image_index], 2)
                seq = np.array(seq)
                # Since we decoded starting with <start>, the targets are all words after <start>, up to <end>
...
train_loader,  val_loader = sampler(0, 128)
# Epochs
for epoch in range(start_epoch, epochs):
    ...
    state = {'epoch': epoch,
             ...
             'encoder_optimizer': encoder_optimizer,
             'decoder_optimizer': decoder_optimizer}
    filename = 'checkpoint_' + 'attention1' + '.pth.tar'
